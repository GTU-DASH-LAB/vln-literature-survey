...
\usepackage{booktabs}
\usepackage{multirow}
% ragged-right text column: justified text in a 2cm column stretches its
% spaces until "Ieong and Tang" breaks into three pieces.
...
benchmark~\cite{anderson2018vision}, VLN has been the canonical setting in
...
attention~\cite{hong2021recurrent}, then with history-aware
...
detector~\cite{liu2024grounding} localises it; a large language model
...
scene~\cite{zhou2024navgpta,dorbala2022clip}; by 2024 the interface has
...
(R2R, RxR~\cite{ku2020room}, REVERIE~\cite{qi2020reverie},
...
Table~\ref{tab:surveys} positions this article against the seven prior
reviews whose scope overlaps ours and whose full text we read. Their
organising axes differ, but they share a shape: each cuts the field along a
\emph{pipeline} or \emph{system-hierarchy} dimension --- which stage of
processing, which level of the control stack, which inference domain. That
choice is defensible and it is not ours, because it makes the property this
survey is about invisible: two systems occupying the same stage can differ
by eight million training samples, and no cell in those taxonomies records
it. We organise by the \emph{architectural role} the foundation model plays
(R1--R5) and encode supervision as an orthogonal attribute, so the two
questions stay separable.

Second, and more consequentially, no prior review audits the zero-shot
claim. Two of the seven make ``zero-shot'' a category heading --- a class
of method alongside end-to-end and modular ones --- which inherits the
ambiguity rather than resolving it; the rest use the term in passing, and
one does not use it at all. It carries at least five mutually incompatible
senses in this literature (Sec.~\ref{sec:zs-defs}), and papers routinely
compare against each other across that gap.

One prior survey should be named rather than counted.
Wang \emph{et al.}~\cite{wang2026comprehensive} is the only review in
Table~\ref{tab:surveys} that measures rather than describes: it tabulates
training compute per system and runs its own real-robot evaluation over ten
scenes, where a monolithic method scoring 61\% in simulation reaches 22\% in
the real world against 51\% for a hierarchical one. That is the kind of
evidence this article argues the field is short of, and
Sec.~\ref{sec:real} reads it alongside the deployments the corpus itself
reports.
...
\begin{figure}[!t]
  \centering
  \resizebox{\columnwidth}{!}{\input{fig/organisation}}
  \caption{Organisation of the survey. Sec.~\ref{sec:zeroshot} is the
  load-bearing chapter; Secs.~\ref{sec:perception}--\ref{sec:policies}
  supply the components it audits.}
  \label{fig:organisation}
\end{figure}

\begin{table*}[!t]
  \caption{Prior surveys of vision-and-language and embodied navigation, and
  what this article adds. Screening routed \ScrSurvey{} records to
  \emph{survey}; listed here are the seven in scope whose full text we read,
  because a comparison table must not assert what a survey omits on the
  strength of its abstract. ``ZS audit'': does the survey distinguish the
  senses in which its sources use ``zero-shot'', rather than repeat the claim
  or use it as a method label? ``Cost'': is compute, latency or
  foundation-model-call count an axis of comparison rather than a future-work
  remark?}
  \label{tab:surveys}
  \centering
  \footnotesize
  \setlength{\tabcolsep}{3.5pt}
  \begin{tabular}{@{}L{2.15cm}cL{2.0cm}L{3.85cm}L{2.25cm}L{2.05cm}L{2.55cm}@{}}
    \toprule
    Survey & Year & Scope & Organising axis & ZS audit & Cost & Real-robot
    evidence \\
    \midrule
    Lin \emph{et al.}~\cite{lin2023advances} & 2023 &
      Embodied navigation with LLMs &
      Role of the LLM: grounded understanding vs.\ few-shot planning &
      No & Named as a challenge & --- \\
    Zhang \emph{et al.}~\cite{zhang2024vision} & 2024 & VLN &
      Top-down: world model, human model, agent model &
      No & --- & Narrative, not tabulated \\
    Ma \emph{et al.}~\cite{ma2024survey} & 2024 &
      VLA across embodied tasks &
      System hierarchy: low-level control policy vs.\ high-level planner &
      No & Narrative & Datasets and benchmarks \\
    Ieong and Tang~\cite{ieong2025multimodal} & 2025 &
      Goal-oriented navigation &
      Inference domain: what the agent reasons over &
      Used as a method class & --- & --- \\
    Xiong \emph{et al.}~\cite{xiong2025sensing} & 2025 &
      Embodied navigation &
      TOFRA pipeline stage: transition, observation, fusion, reward, action &
      No & FLOPs listed among metrics & --- \\
    Wang \emph{et al.}~\cite{wang2025safety} & 2025 &
      Embodied navigation safety &
      Attack, defence, evaluation &
      Term not used & --- & --- \\
    Wang \emph{et al.}~\cite{wang2026comprehensive} & 2026 & VLN &
      Action paradigm (hierarchical vs.\ monolithic) $\times$ model paradigm
      (discriminative vs.\ generative) &
      Used as a strategy class & GPU-hours tabulated; inference time measured
      & Own experiments, 10 scenes \\
    \midrule
    This article & 2026 & VLN and object-goal navigation &
      Role of the foundation model (R1--R5), with supervision as an
      orthogonal attribute &
      Yes (ZS-1--ZS-5, Table~\ref{tab:zsaudit}) &
      Yes (Table~\ref{tab:zsvssup}) &
      Corpus-wide (Table~\ref{tab:realrobot}) \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{figure*}[!t]
  ...
  \resizebox{\textwidth}{!}{\input{fig/selection-flow}}
...
ConceptGraphs~\cite{gu2024conceptgraphs}, Code as
Policies~\cite{liang2023code} --- never use ``navigation'' in the title at
...
\begin{figure*}[!t]
  \centering
  \resizebox{\textwidth}{!}{\input{fig/taxonomy}}
...
\begin{figure*}[!t]
  \centering
  \begin{tabular}{@{}p{0.485\textwidth}@{\hspace{4mm}}p{0.485\textwidth}@{}}
    \centering\includegraphics[width=0.972\linewidth]{fig/arch-a-lgx.png} &
    \centering\includegraphics[width=0.913\linewidth]{fig/arch-b-navgpt2.jpg}
    \tabularnewline[1mm]
    \centering\footnotesize (a) R1: the reasoner is outside the loop
      (LGX~\cite{dorbala2024embodied}) &
    \centering\footnotesize (b) R2: the model perceives, a trained policy acts
      (NavGPT-2~\cite{zhou2024navgpt})
    \tabularnewline[1.5mm]
    \centering\includegraphics[width=0.972\linewidth]{fig/arch-c-beliefmap.png} &
    \centering\includegraphics[width=0.972\linewidth]{fig/arch-d-omninav.png}
    \tabularnewline[1mm]
    \centering\footnotesize (c) the map is what the reasoner writes into
      (BeliefMapNav~\cite{zhou2025beliefmapnav}) &
    \centering\footnotesize (d) R3: the policy absorbs the reasoner and learns
      when to call it (OmniNav~\cite{xue2025omninav})
    \tabularnewline
  \end{tabular}
  \caption{The taxonomy of Fig.~\ref{fig:taxonomy} in the authors' own
  drawings. Read (a) to (d) as the argument of this survey in four pictures:
  the reasoner begins as an external module invoked every step (a), becomes a
  perception front-end for a trained policy (b), acquires a persistent spatial
  substrate to reason over (c), and is finally absorbed into the policy itself,
  which learns when the expensive process is worth running (d). Panels
  reproduced from the cited works under CC~BY~4.0 (a, c, d) and CC~BY-SA~4.0
  (b); see \texttt{latex/fig/CREDITS.md} in the accompanying repository.}
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.92\textwidth]{fig/datagen-simworld.jpg}
  \caption{The R4 role closed into a loop. A code-writing model turns a text
  prompt into a scene, the render is scored by a multimodal verifier against
  an explicit rubric --- visual quality, layout coherence, physical
  plausibility, diversity --- and the scene is regenerated until it passes,
  after which an embodied agent is asked to act in it. Both the author and
  the critic of the training data are foundation models, which is what makes
  the inheritance question of Sec.~\ref{sec:challenges} structural rather
  than a matter of data volume. Reproduced from~\cite{kang2026simworld}
  under CC~BY~4.0.}
  \label{fig:datagen}
\end{figure*}
...
with a multimodal verifier in the loop (Fig.~\ref{fig:datagen}),
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.86\textwidth]{fig/goals-goat.jpg}
  \caption{Why the goal specification is the harder variable. A single
  GOAT-Bench episode sequence asks for the same apartment to be navigated
  seven times, with the goal given as a category (\emph{recliner chair}), as
  a photograph, and as a referring expression (\emph{the white book on the
  coffee table in the living room}) --- so an agent cannot specialise to one
  goal modality, and open-vocabulary competence (ZS-2) is measured rather
  than assumed. Reproduced from~\cite{khanna2024goat} under CC~BY~4.0.}
  \label{fig:goals}
\end{figure*}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{llllll}
    ...
    REVERIE~\cite{qi2020reverie}              & Benchmark & Indoor & Object + coarse hint & --- & Remote object grounding \\
...
detection~\cite{liu2024grounding} and promptable
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig/video-navid.jpg}
  \caption{The monocular setting, in a real office rather than a scan. The
  upper panel is the trajectory as an observer sees it; the lower strip is
  everything the agent receives --- one forward-facing RGB video stream,
  with no depth, no map, no odometry and no panorama.
  NaVid~\cite{zhang2024navid} consumes that strip directly. Reproduced
  from~\cite{zhang2024navid} under CC~BY~4.0.}
  \label{fig:navid}
\end{figure}
...
video stream (Fig.~\ref{fig:navid}), and NavFoM~\cite{zhang2025embodied} reports the size of the
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig/scene-toponav.jpg}
  \caption{What a topological memory buys the reasoner. Each node carries a
  room type, the objects observed there and the number of frontiers it still
  has, so the prompt states where the agent has already been. Without that
  memory (upper text box) the model reasons from the current view alone and
  re-enters an explored bedroom (red waypoint); with it, the same model
  notes that the bedroom node held no bathroom and routes to the unexplored
  frontiers of node~5 (green waypoint). The failure being repaired is not
  perception but memory. Reproduced from~\cite{liu2025toponav} under
  CC~BY~4.0.}
  \label{fig:toponav}
\end{figure}
...
backtracking expressible and what lets the reasoner see its own history
(Fig.~\ref{fig:toponav}). \emph{Sparse geometric abstractions} such as the
...
originates outside navigation. Visual language maps~\cite{huang2023visual}, open-vocabulary queryable
scene representations~\cite{chen2023open}, open-set multimodal
mapping~\cite{jatavallabhula2023conceptfusion} and open-vocabulary 3D scene
graphs~\cite{gu2024conceptgraphs} make a map queryable by arbitrary text.
...
BERT~\cite{hong2021recurrent}, history-aware
...
agents~\cite{hong2021recurrent}, history-aware
transformers~\cite{chen2021history} and topological graph
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig/deploy-navila.jpg}
  \caption{The two-level structure shared by every deployed VLA in the
  corpus. NaVILA~\cite{cheng2024navila} reads the instruction together with
  the current and history views and emits a mid-level command in natural
  language (\emph{move forward 75\,cm}); a separate locomotion policy,
  running far faster on proprioception and a local height map, turns that
  command into joint positions. The foundation model never produces a
  torque, and the two levels are trained on different data --- which is why
  ``trained'' and ``training-free'' are properties of a layer rather than of
  a system (Sec.~\ref{sec:zs-defs}). Reproduced from~\cite{cheng2024navila}
  under CC~BY~4.0.}
  \label{fig:navila}
\end{figure*}
...
executes them (Fig.~\ref{fig:navila}), which is what allows the same policy
to drive a legged robot. OmniNav~\cite{xue2025omninav} uses a fast--slow decomposition to hold
...
search~\cite{gadre2023cows} established that an open-vocabulary detector plus
...
outside navigation; LangNav~\cite{pan2024langnav} took the extreme position
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig/hydra-dualprocess.jpg}
  \caption{The 3.0\% figure, drawn. A fast system emits low-level actions
  (green) from the current view; the slow system reasons over long-term
  memory (red) only when the agent decides it must, and the context is
  cleared at each switch. Memory is written back as text plus a landmark
  image rather than carried in the prompt, which is what keeps the slow
  path affordable. The deliberation that defined the 2023--2024 zero-shot
  agents survives here as a subroutine invoked on 3.0\% of steps.
  Reproduced from~\cite{wang2026hydra} under CC~BY~4.0.}
  \label{fig:hydra}
\end{figure*}
...
Hydra-Nav~\cite{wang2026hydra} learns when to think (Fig.~\ref{fig:hydra}),
reducing the reasoning ratio to 3.0\% of steps; SURF~\cite{shinnick2026surf} triggers on
...
\begin{figure}[!t]
  \centering
  \resizebox{0.95\columnwidth}{!}{\input{fig/zs-loop}}
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.86\textwidth]{fig/eval-navitrace.jpg}
  \caption{The gap that success rate hides. NaviTrace scores a model's
  proposed trajectory against human demonstrations across a thousand scenes,
  each annotated with the constraint it tests --- accessibility, geometric
  terrain, social norms --- rather than against a distance threshold. The
  best multimodal model on the leaderboard reaches 34.55 against a human
  expert's 75.04, and every model listed is one that reports high success on
  the indoor benchmarks of Sec.~\ref{sec:eval}. Reproduced
  from~\cite{windecker2025navitrace} under CC~BY~4.0.}
  \label{fig:navitrace}
\end{figure*}
...
task success (Fig.~\ref{fig:navitrace}). This is the most useful corrective in the corpus to the
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig/embodiment-vamos.jpg}
  \caption{What ``cross-embodiment'' (ZS-5) means once the agent leaves the
  simulator. VAMOS~\cite{castro2025vamos} keeps one language-conditioned
  planner and swaps only the affordance model, which is what encodes that a
  legged base may cross terrain a wheeled one cannot; the plan is then
  filtered rather than retrained. The right-hand panel is the setting the
  scanned indoor benchmarks of Sec.~\ref{sec:data} do not contain.
  Reproduced from~\cite{castro2025vamos} under CC~BY~4.0.}
...
\begin{figure*}[!t]
  \centering
  \begin{tabular}{@{}p{0.485\textwidth}@{\hspace{4mm}}p{0.485\textwidth}@{}}
    \centering\includegraphics[height=62mm]{fig/deploy-urbanvla.jpg} &
    \centering\includegraphics[height=62mm]{fig/deploy-navfom.jpg}
    \tabularnewline[1mm]
    \centering\footnotesize (a) The parts a benchmark episode has no name for
      (UrbanVLA~\cite{li2025urbanvla}) &
    \centering\footnotesize (b) One set of weights, four action spaces
      (NavFoM~\cite{zhang2025embodied})
  \end{tabular}
  \caption{What deployment adds that Table~\ref{tab:realrobot} cannot show.
  (a)~UrbanVLA names the compute, the localisation and the network link as
  first-class components, and adds an operator console --- start, stop,
  crash, fail --- that no simulator episode has an analogue for.
  (b)~NavFoM tokenises instruction and observation identically for a wheeled
  base, a drone, a quadruped and a surround-view car, which is the concrete
  form ZS-5 takes. Reproduced from~\cite{li2025urbanvla}
  and~\cite{zhang2025embodied} under CC~BY~4.0.}
  \label{fig:deploy}
\end{figure*}

\subsection{Reported Deployments}
Table~\ref{tab:realrobot} lists deployments reported in the corpus and
Fig.~\ref{fig:deploy} shows two of them. Note the pattern: a large share of the real-robot demonstrations are zero-shot
